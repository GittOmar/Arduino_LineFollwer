...
void loop() 
{
  ...
  if(IRstate_Middle == HIGH)      // First Case _ IF Middle See Black Move Forward    // IF
   {
    Forward();
   }
 
  else if(IRstate_Middle == LOW)  // Second Case _ Middle See White                  // Esle IF
   {
  while(IRstate_Right == LOW && IRstate_Left == LOW) //  While Right And Left See White Move Forward  // While
   {
    Forward();
   }
   ...
   { 
   while(IRstate_Middle == LOW) // While Middle See White Turn Left   // While
   {
    Left();
    tone(Buz,2900,100);
   }
    ...
    {
   while(IRstate_Middle == LOW) // While Middle See White Turn Right  // While
    { 
      Right();
      tone(Buz,3000,100);
    }
...
void Right()
{
  ...
  analogWrite (Pwm_Right,Main_Speed);
  analogWrite (Pwm_Left ,backF); 
  
  digitalWrite(F_Right,1);
  digitalWrite(B_Right,0);
  
  digitalWrite(F_Left ,0);
  digitalWrite(B_Left ,1);
}
...
void Left()
{
  ...
  analogWrite (Pwm_Left ,Main_Speed);
  analogWrite (Pwm_Right,backF); 

  digitalWrite(F_Left ,1);
  digitalWrite(B_Left ,0);
  
  digitalWrite(F_Right,0);
  digitalWrite(B_Right,1);
...
void SpeedCalculations()
{
  ...
  Secondary_Speed       = (Potentiometer_Read*255.0)/1023.0;
